...
plotHeatmap <- function(geneExp, group, myPalette, legendName="Gene expression", method="ward.D2", distance="spearman",
                        s1=15, s2=8, s3=12, s4=6, s5=12, H, W, title="Heatmap", Date) {
  ...
  ht <- Heatmap(geneExp, cluster_row_slices = FALSE, cluster_column_slices = FALSE, row_labels = row.names(geneExp),
                ...
                show_row_names = FALSE, clustering_distance_rows = distance, clustering_distance_columns = distance,
                ...
                show_heatmap_legend = FALSE, row_dend_width = unit(s5, "mm"), column_dend_height = unit(s5, "mm"), use_raster = FALSE)
